...
def predict():
    ...
    if(k['neu']==1.0):
        ...
        if((k['neg']>k['pos']) and ( (k['neg']-k['pos'])<=0.24 and (k['neg']-k['pos'])>=0  ) ):
            ...
            rating=5
    remove_lower_punct =int_features.lower().replace(r'[^\w\s]', ' ').replace('.','').replace(',','').replace(r" \d+", " ").replace(r' +', ' ').strip()
    ...
    return render_template('predict.html',predict_text='Review is: {}'.format(ans),score=rating,aspects=aspects,int_features='Entered Review: {}'.format(int_features))
...
def analysis():
    ...
    df['remove_lower_punct'] = df['reviews'].str.lower().str.replace(r'[^\w\s]', ' ').str.replace(r" \d+", " ").str.replace(r' +', ' ').str.strip()
